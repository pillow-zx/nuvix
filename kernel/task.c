...
static void task_init_common(struct task_struct *task)
{
	...
	cpumask_zero(&task->requested_affinity);
	cpumask_set_cpu(&task->requested_affinity, SCHED_BOOT_AFFINITY_CPU);
	cpumask_copy(&task->effective_affinity, &task->requested_affinity);
	task->cpu = NULL;
	task->on_rq = false;
	task->on_cpu = false;
