...
static_assert(sizeof(struct uaccess_exception_entry) == 2 * sizeof(uintptr_t),
	      "uaccess exception entry must contain two addresses");

extern const struct uaccess_exception_entry __start___ex_table[];
extern const struct uaccess_exception_entry __stop___ex_table[];

static bool uaccess_fault_cause(uintptr_t cause)
{
	cause &= ~SCAUSE_IRQ_FLAG;

	return cause == EXC_LOAD_PAGE_FAULT || cause == EXC_STORE_PAGE_FAULT ||
	       cause == EXC_LOAD_ACCESS || cause == EXC_STORE_ACCESS;
}

bool uaccess_fixup(struct trap_frame *tf)
{
	const struct uaccess_exception_entry *entry;
