...
static struct cpu_topology_entry topology[NR_CPUS];
...
{
	int cpus = fdt_path_offset(dt_blob, "/cpus"), node, len;
	uint32_t freq;
	bool boot_found = false, sstc = true;
	...
	topology[0] = (struct cpu_topology_entry){0, (uint32_t)boot_hartid};
	cpu_count = 1;
	fdt_for_each_subnode(node, dt_blob, cpus) {
		...
		if (hart == boot_hartid) {
			...
			topology[cpu_count] = (struct cpu_topology_entry){cpu_count, (uint32_t)hart};
			cpu_count++;
#endif
		}
